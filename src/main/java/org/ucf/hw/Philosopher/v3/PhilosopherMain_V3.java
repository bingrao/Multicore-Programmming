package org.ucf.hw.Philosopher.v3;
/**
 * @author Bing
 *
 */
...
class Waitor
{
	...
	Waitor()
	{
		chopsticks = new ShareChopstick[5];
		for(int i=0;i<5;i++)
		{
	...
	public synchronized void require_chopsticks(int p_idx)
	{
		...
		myslot.set(slot);
		while((!flag[slot])&&(chopsticks[p_idx].get_chopstick_status()||chopsticks[(p_idx+1)%5].get_chopstick_status()))
		{
	...
	public synchronized void release_chopsticks(int p_idx)
	{
		...
		this.flag[slot%this.size] = false;
		this.flag[(slot+1)%this.size] = true;
